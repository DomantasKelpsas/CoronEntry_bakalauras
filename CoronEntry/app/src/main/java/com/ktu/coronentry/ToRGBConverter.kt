...
class ToRGBConverter (context: Context) {
    private val rs = RenderScript.create(context)
    private val scriptToRGB = ScriptIntrinsicYuvToRGB.create(rs, Element.U8_4(rs))
    ...
    @Synchronized
    fun convToRGB(image: Image, output: Bitmap)
    {
        ...
        yuvBuffer.rewind()
        imgToByteBuffer(image,yuvBuffer.array())
        ...
        scriptToRGB.setInput(inputAllocation)
        scriptToRGB.forEach(outputAllocation)
        outputAllocation.copyTo(output)
    }

    private fun imgToByteBuffer(image: Image, outputBuffer: ByteArray)
    {
